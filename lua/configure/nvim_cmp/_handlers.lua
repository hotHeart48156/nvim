...
handlers.setup = function()
    ...
    for _ , sign in ipairs(signs) do
        vim.fn.sign_define(sing.name,{texthl=sign.name,text=sign.text,numhl=""})
    end
    local config={
        virtual_text=false,
        signs={
    ...
    vim.lsp.handlers['textDocument/signtureHelp']=vim.lsp.with(vim.lsp.handlers.signature_help,{border='rounder'})
end
...
local function lsp_key_maps(buf)
    ...
    vim.api.nvim_buf_set_keymap(buf,'n','<leader>dj','<cmd>lua vim.diagnostic.goto_prev({border="rounded"})<CR>',opts)
    vim.api.nvim_buf_set_keymap(buf,'n','<leader>dk','<cmd>lua vim.diagnostic.goto_next({border="rounded"})CR>',opts)
end
